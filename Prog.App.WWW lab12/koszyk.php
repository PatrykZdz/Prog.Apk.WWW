...
function DodajKoszyk($id, $ilosc)
{
    ...
    {
        $zapytanie = $link->prepare("SELECT id, tytul, cena_netto, podatek_vat FROM
...
{
    include('cfg.php');
    $wynik = $link->query("SELECT id, tytul, cena_netto, podatek_vat FROM produkty");
...
if($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    elseif($action === 'update')
    {
        ...
        {
            $_SESSION['koszyk'][$produktId]['Ilosc'] = max(1, $ilosc);  // Ustawiamy minimalną ilość na 1
        }
    }
}
?>
